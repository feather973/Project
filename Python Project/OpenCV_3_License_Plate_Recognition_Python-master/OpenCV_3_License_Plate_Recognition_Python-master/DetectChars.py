...
# important things that detect contour works
MIN_PIXEL_WIDTH = 10 #2 #2
MIN_PIXEL_HEIGHT = 10 #2 #2
MAX_PIXEL_WIDTH = 100 #100
MAX_PIXEL_HEIGHT = 140 #100
MIN_ASPECT_RATIO = 0.5 #0.1 #0.1
MAX_ASPECT_RATIO = 3 #1.94 #3.5
MIN_PIXEL_AREA = 50 #180 #30
# 구형 번호판의 경우, 최대 종횡비가 커야 한글을 인식 가능하다.
# 하지만 종횡비가 클 경우 번호판 모양(사각형) 까지 인식하게 되고 나아가 나사 모양까지 인식하게 된다.
# 번호판 모양의 경우, 최대 가로세로 길이 설정을 통하여 지정하면 된다.
# 나사 모양의 경우, 답이 없다

# constants for comparing two chars
MIN_DIAG_SIZE_MULTIPLE_AWAY = 0.2 #0.3
MAX_DIAG_SIZE_MULTIPLE_AWAY = 5.0 #9.0
MAX_CHANGE_IN_AREA = 1 #0.4
MAX_CHANGE_IN_WIDTH = 2 #0.2
MAX_CHANGE_IN_HEIGHT = 0.8 #0.8
MAX_ANGLE_BETWEEN_CHARS = 3.0 #3.0
...
def detectCharsInPlates(listOfPossiblePlates):
    ...
    for possiblePlate in listOfPossiblePlates:          # for each possible plate, this is a big for loop that takes up most of the function
        ...
        thresholdValue, possiblePlate.imgThresh = cv2.threshold(possiblePlate.imgThresh, 0.0, 255.0, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)
        #if Main.showSteps == True: # show steps ###################################################
...
def checkIfPossibleChar(possibleChar):
    ...
    if (possibleChar.intBoundingRectArea > MIN_PIXEL_AREA and
        ...
        MIN_ASPECT_RATIO < possibleChar.fltAspectRatio and possibleChar.fltAspectRatio < MAX_ASPECT_RATIO and
        possibleChar.intBoundingRectWidth < MAX_PIXEL_WIDTH and possibleChar.intBoundingRectHeight < MAX_PIXEL_HEIGHT):
        return True
    else:
        return False
     #end if
...
def recognizeCharsInPlate(imgThresh, listOfMatchingChars):
    ...
    # 숫자 2개, 한글1개, 숫자 4개 순서이므로 특정한 조건 때 tesseract를 이용하여 한글을 읽도록 함.
    plateList = []
    for currentChar in listOfMatchingChars:                                         # for each char in plate
        ...
        cv2.imshow("processing", imgThreshColor)
        cv2.waitKey(0)
        ...
        # 글자 인식 과정
        cv2.imshow("current_image", imgROI)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
        config = ('-l kor --oem 1 --psm 10')
        strCurrentChar = pytesseract.image_to_string(imgROI, config=config)
        strChars = strChars + strCurrentChar  # append current char to full string
      #  print("current strChars " + strChars)

    # end for
    plateList.append(strChars)
