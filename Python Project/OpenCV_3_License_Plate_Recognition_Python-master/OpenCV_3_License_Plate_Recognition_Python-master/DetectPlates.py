...
PLATE_WIDTH_PADDING_FACTOR = 3
PLATE_HEIGHT_PADDING_FACTOR = 4
...
def detectPlatesInScene(imgOriginalScene):
    ...
    if Main.showSteps == True: # show steps #######################################################
       # print("step 2 - len(listOfPossibleCharsInScene) = " + str(
        ...
        # end for
        cv2.drawContours(imgContours, contours, -1, Main.SCALAR_RED, 2)
        cv2.imshow("fixed imgContours", imgContours)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
    # end if # show steps #########################################################################
...
def findPossibleCharsInScene(imgThresh):
    ...
    for i in range(0, len(contours)):                       # for each contour
        if Main.showSteps == True: # show steps ###################################################
            # 컨투어를 그림 -> 아직 주변 잡티가 제거되지 않은 상태
            cv2.drawContours(imgContours, contours, i, Main.SCALAR_WHITE)
        # end if # show steps #####################################################################
        cv2.imshow("Original imgContours", imgContours)
