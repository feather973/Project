...
volatile int i=0;   // buffer_uart0 의 index 변수
volatile unsigned char buffer_0[MAX_QSIZE]={0};   // UART 부터 읽어온 data를 \r 이나 \n을 만날때 까지 저장할 공간
volatile int rxReadyFlag_0=0;   // \r이나 \n을 만났다는 인디케이터 flag변수

unsigned char *getRxString_0()
{
	rxReadyFlag_0 = 0;   // 또 다른 message를 처리 하기 위하여 rxReadyFlag_uart0를 clear한다.
	return buffer_0;
...
void UART0_init(void)
{
	...
	UCSR0B = ((1<<RXEN0)|(1<<TXEN0) | (1 << RXCIE0));   // #2. 송.수신 가능 하고록 설정을 하는것이다. 
			 ...
			 //  RXEN0 : 수신을 가능 하도록 설정  	TXEN1: 송신을 가능 하도록 설정  
}
/*
