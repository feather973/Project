...
const char* ssid     = "KCCI_STC_S"; // Assign the SSID name
const char* password = "kcci098#"; // Assign the SSID password

const char* host = "dkals0805.000webhostapp.com";
...
bool isThereEnemy(){
  ...
  for(int i=0;i<2;i++){
    getXY(i, ang);
    if(measure[i]<=80 && measure[i] >=2) flag = true;
    else {
      // No Enemy -> (900, 900)
      pack[i].x = 900;
      pack[i].y = 900;
  ...
  char url[100];
  sprintf(url, "/TX.php?id=%d&en1x=%d&en1y=%d&en2x=%d&en2y=%d",
  ang, pack[0].x, pack[0].y, pack[1].x, pack[1].y);
...
void loop() {
  ...
  if(isThereEnemy()){
    pack[0].x = abs(pack[0].x);
    pack[0].y = abs(pack[0].y);
    pack[1].x = abs(pack[1].x);
    pack[1].y = abs(pack[1].y);
