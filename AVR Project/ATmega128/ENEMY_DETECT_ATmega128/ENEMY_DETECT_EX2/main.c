...
// struct for Enemies coordinate and Quadrant Number
typedef struct{
	int x;
	int y;
	int q;
...
void killEnemy(int idx){
	...
	switch(Enemy[idx].q){
		case 1:{
			ang1 = (int)(ceil(atan2(Enemy[idx].y - NSL, Enemy[idx].x) * 180 / PI));
			ang2 = (int)(180 - ceil(atan2(Enemy[idx].x - EWL, Enemy[idx].y) * 180 / PI));
			break;
		}
		case 2:{
			ang1 = (int)(abs(ceil(atan2(Enemy[idx].x + EWL, Enemy[idx].y) * 180 / PI)));
			ang2 = (int)(ceil(atan2(Enemy[idx].y -NSL, Enemy[idx].x) * 180 / PI));
			break;
		}
		case 3:{
			ang1 = (int)(180 - abs(ceil(atan2(Enemy[idx].y + NSL, Enemy[idx].x) * 180 / PI)));
			ang2 = (int)abs(ceil(atan2(Enemy[idx].x + EWL, Enemy[idx].y) * 180 / PI));
			break;
		}
		case 4:{
			ang1 = (int)(ceil(180 - atan2(Enemy[idx].x - NSL, Enemy[idx].y) * 180 / PI));
			ang2 = (int)(ceil(180 + atan2(Enemy[idx].y + EWL, Enemy[idx].x) * 180 / PI));
			break;
		}
		default:break;
	}
